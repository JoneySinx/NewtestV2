...
async def actor_profile_display(req):
  ...
  /* ── Edit modal ── */
  .edit-modal{{display:none;position:fixed;inset:0;background:rgba(0,0,0,.7);backdrop-filter:blur(6px);z-index:9999;align-items:center;justify-content:center;padding:20px;box-sizing:border-box}}
  .edit-modal.open{{display:flex}}
  .em-card{{position:relative;width:100%;max-width:550px;max-height:90vh;overflow-y:auto;background:var(--card);border:1px solid var(--border);padding:25px;border-radius:12px}}
</style>
...
<script>
  ...
  async function updateActorAvatar(actorId){{
    ...
    var label=fi.closest('label');
    if(label){{label.style.opacity='0.5';label.style.pointerEvents='none';label.textContent='⏳ Uploading...'}}
    var fd=new FormData();
    ...
    finally{{if(label){{label.style.opacity='';label.style.pointerEvents='';label.textContent='📸 Change Avatar'}}}}
  }}
  ...
  document.getElementById('actor_movie_q').addEventListener('keydown',function(e){{if(e.key==='Enter'){{resetActorSearchPage();triggerActorSearchAjax()}}}});
    ...
    return build_page(f"{html.escape(actor_name)} - Profile", page, "", "actors", role)
...
async def api_actor_update_profile(req):
    ...
    try:
        ObjectId(actor_id)  # validate format
    except Exception:
        return web.HTTPFound('/actors?err=Invalid actor ID')
    ...
    await actors.update_one({"_id": ObjectId(actor_id)}, {"$set": {
        "name": name, "bio": bio, "tags": tags_list,
        "social_links": {
            "instagram": safe_url(d.get('insta','')),
            "youtube":   safe_url(d.get('yt','')),
            "twitter":   safe_url(d.get('twitter',''))
        }
    }})
